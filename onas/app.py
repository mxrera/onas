...
class App(ctk.CTk):
    ...
    def __init_settings_tab(self):
        ...
        ctk.CTkComboBox(self.tabview.tab("settings"), values=["JPEG"], command=self.on_codification_select, fg_color=BG_COLOR).grid(row=3, column=0, sticky=ctk.W + ctk.E)
        ctk.CTkLabel(self.tabview.tab("settings"), text="Codification Options").grid(row=4, column=0, sticky=ctk.W + ctk.E)
        self.options_frame = ctk.CTkScrollableFrame(self.tabview.tab("settings"), fg_color=FG_COLOR).grid(row=5, column=0, sticky=ctk.N + ctk.E + ctk.S + ctk.W)
    ...
    def on_codification_select(self, codification):
        if codification == "JPEG":
            ...
            self.codification_options = self.codification.options()
            self.fill_codification_options()
    ...
    def fill_codification_options(self):
        for widget in self.options_frame.winfo_children():
            widget.destroy()

        for key, value in self.codification_options.items():
            if value["type"] == "combobox":
                ctk.CTkComboBox(self.options_frame, values=value["values"], fg_color=BG_COLOR).pack()
            elif value["type"] == "spinbox":
                ctk.CTkSpinBox(self.options_frame, values=value["values"], fg_color=BG_COLOR).pack()
            elif value["type"] == "button":
                ctk.CTkButton(self.options_frame, text="Select", fg_color=BG_COLOR)
